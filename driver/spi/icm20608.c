...
#define ICM20608_CHAN(_type, _channel2, _index)                    \
	...
	}//info_mask_separate 标记某些属性专属于此通道

/* 
 * ICM20608的扫描元素，3轴加速度计、
 * 3轴陀螺仪、1路温度传感器，1路时间戳  用于填充数据
 */
...
static const struct iio_chan_spec icm20608_channels[] = {
	/* 温度通道 */
	{
		.type = IIO_TEMP,//温度类型的通道，有各种各样的通道，比如说电流电压加速度等
		.info_mask_separate = BIT(IIO_CHAN_INFO_RAW)
    ...
    //IIO_ANGL_VEL角速度通道，IIO_ACCEL加速度通道
...
{
    int ind, result;
    __be16 d = cpu_to_be16(val);  // 将 val 转换为大端字节序的 16 位数据

    ind = (axis - IIO_MOD_X) * 2; // 计算寄存器偏移量，因为每个两字节
    result = regmap_bulk_write(dev->regmap, reg + ind, (u8 *)&d, 2); // 写入 2 字节数据
    if (result)
        return -EINVAL; // 写入失败，返回错误
...
static int icm20608_write_gyro_scale(struct icm20608_dev *dev, int val)
{
    ...
    // 遍历陀螺仪分辨率数组，找到匹配的 val（分辨率）
    for (i = 0; i < ARRAY_SIZE(gyro_scale_icm20608); ++i) {
        if (gyro_scale_icm20608[i] == val) {
            // 计算寄存器配置值（i << 3）
            d = (i << 3);
            // 在 ICM20_GYRO_CONFIG寄存器中，​​陀螺仪量程由 bit3 和 bit4 (FS_SEL[1:0]) 控制​​。
			//当i=3时，0001 1000  对于第三量程
            result = regmap_write(dev->regmap, ICM20_GYRO_CONFIG, d);
            if (result)
                return result; // 写入失败，返回错误
            return 0; // 写入成功，返回 0
        }
    }
    // 如果 val 不在支持的分辨率列表中，返回 -EINVAL（无效参数）
    return -EINVAL;
}
			   ...
			   int *val, int *val2, long mask)
{
	...
	case IIO_CHAN_INFO_SCALE:
		...
		case IIO_ANGL_VEL://陀螺仪
			mutex_lock(&dev->lock);
			//>> 3将结果右移3位，得到 0、1、2或 3，对应不同的量程（如 ±250°/s、±500°/s 等）。
			regdata = (icm20608_read_onereg(dev, ICM20_GYRO_CONFIG) & 0X18) >> 3;
			*val  = 0;
			*val2 = gyro_scale_icm20608[regdata];//​从 gyro_scale_icm20608数组获取缩放比例
			mutex_unlock(&dev->lock);
			return IIO_VAL_INT_PLUS_MICRO;	/* 用于告诉 IIO 子系统数据为，​带有小数部分的数值​​,值为val+val2/1000000 */

		case IIO_ACCEL:
			mutex_lock(&dev->lock);
			regdata = (icm20608_read_onereg(dev, ICM20_ACCEL_CONFIG) & 0X18) >> 3;//得到不同的量程
			//ICM20_ACCEL_CONFIG（加速度计配置寄存器）的 ​​bit3 和 bit4​​ 用于设置 ​​加速度计量程
			*val = 0;
			*val2 = accel_scale_icm20608[regdata];;
			//该数组 accel_scale_icm20608[] = {61035, 122070, 244140, 488281};
			mutex_unlock(&dev->lock);
			    ...
			    int val, int val2, long mask)
{
	...
	case IIO_CHAN_INFO_SCALE:	/* 设置陀螺仪和加速度计的分辨率 */
		...
		case IIO_ACCEL:			/* 设置加速度计 */
			...
			//函数几乎一模一样，唯有寄存器区别，这个是ICM20_ACCEL_CONFIG
			mutex_unlock(&dev->lock);
			break;
		default:
			ret = -EINVAL;
...
/*
  * @description     	: 比如我们在用户空间操作sysfs来设置传感器的分辨率，如果分辨率带小数，
                         那么这个小数传递到内核空间应该扩大多少倍，此函数就是用来设置这个的。
  * @param - indio_dev	: iio_dev
  * @param - chan   	: 当前操作的传感器通道（如陀螺仪X轴）
  * @param - mask   	: 属性类型（如 IIO_CHAN_INFO_SCALE表示缩放因子）
  * @return				: 0，成功；其他值，错误
  */
...
static const struct iio_info icm20608_info = {
	.read_raw		= icm20608_read_raw, //读数据
	.write_raw		= icm20608_write_raw,//写数据
	.write_raw_get_fmt = &icm20608_write_raw_get_fmt,	
	/*  声明用户空间写数据格式
	         内核先调用 write_raw_get_fmt确认格式（返回 IIO_VAL_INT_PLUS_MICRO）。
             将字符串 "1.23"解析为 val=1, val2=230000。 
             调用 write_raw执行实际写入操作。
	 */
};
