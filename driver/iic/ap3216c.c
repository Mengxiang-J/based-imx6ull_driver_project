...
static const struct iio_chan_spec ap3216c_channels[] = {
    /* ALS (环境光传感器) 通道配置 */
    {
        .type = IIO_INTENSITY,          // 通道类型：光强度测量
        .modified = 1,                  // 启用修饰符（需配合.channel2使用）
        .channel2 = IIO_MOD_LIGHT_BOTH, // 修饰符：测量可见光+红外光组合强度
        .address = AP3216C_ALSDATALOW,  // 寄存器地址（低字节）

        .info_mask_separate =           // 独立属性掩码（每个通道单独存在）
            BIT(IIO_CHAN_INFO_RAW) |    // 提供原始值读取接口，会在bus/总线下出现接口
            BIT(IIO_CHAN_INFO_SCALE),   // 表示量程缩放因子

        .scan_index = AP3216C_ALS,      // 缓冲区中的扫描索引（用于数据流排序）
        .scan_type = {                  // 缓冲区数据格式定义
            .sign = 'u',                 // 数据类型：无符号（'u'=unsigned, 's'=signed）
            .realbits = 16,              // 有效数据位数（AP3216C的ALS输出为16位）
            .storagebits = 16,          // 存储位数（与realbits相同，无填充）
            .endianness = IIO_LE,       // 字节序：小端（与传感器数据格式匹配）
        },
    },

    /* PS (接近传感器) 通道配置 */
    {
        .type = IIO_PROXIMITY,          // 通道类型：接近检测
        .address = AP3216C_PSDATALOW,   // 寄存器地址（低字节）
        .info_mask_separate =           // 独立属性掩码
            BIT(IIO_CHAN_INFO_RAW),     // 仅提供原始值接口（in_proximity_raw）
        .scan_index = AP3216C_PS,       // 缓冲区中的扫描索引
        .scan_type = {
            .sign = 'u',                // 无符号数据
            .realbits = 10,             // 有效位数（AP3216C的PS输出为10位）
            .storagebits = 16,          // 存储为16位（高位填充0）
            .endianness = IIO_LE,       // 小端模式
        },
    },

    /* IR (红外光) 通道配置 */
    {
        .type = IIO_INTENSITY,          // 通道类型：光强度
        .modified = 1,                  // 启用修饰符
        .channel2 = IIO_MOD_LIGHT_IR,   // 修饰符：仅测量红外光部分
        .address = AP3216C_IRDATALOW,   // 寄存器地址（低字节）
        .info_mask_separate =           // 独立属性掩码
            BIT(IIO_CHAN_INFO_RAW),     // 提供原始值接口（in_intensity_ir_raw）
        .scan_index = AP3216C_IR,       // 缓冲区中的扫描索引
        .scan_type = {
            .sign = 'u',                // 无符号数据
            .realbits = 10,             // 有效位数（AP3216C的IR输出为10位）
            .storagebits = 16,          // 存储为16位（兼容缓冲区对齐）
            .endianness = IIO_LE,       // 小端模式
        },
    },
};
...
static const unsigned long ap3216c_scan_masks[] = {
	BIT(AP3216C_ALS)
	| BIT(AP3216C_PS)  //// 同时启用通道AP3216C_ALS和AP3216C_PS
	| BIT(AP3216C_IR),//或者仅启动AP3216C_IR
	0,
};
...
static int ap3216c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/*  1、申请iio_dev内存，获取ap3216c_dev结构体地址 */
	...
	indio_dev->available_scan_masks = ap3216c_scan_masks;//定义哪些通道可以组合启用（用于缓冲模式）
