...
/* 关键：实时帧率计算和统计 */
static void ov5640_fps_update(struct ov5640_dev *sensor)
{
    ktime_t now = ktime_get();
    time_diff = ktime_to_ms(ktime_sub(now, stats->last_frame_time));
    if (time_diff > 0)
        stats->current_fps = 1000 / time_diff;
}
/* 关键：调试信息导出 */
static int ov5640_debugfs_regs_show(struct seq_file *s, void *data)
{
    // 读取并显示重要寄存器
    ret = ov5640_read_reg(sensor, important_regs[i].reg, &val);
    seq_printf(s, "0x%04x %-20s: 0x%02x\n", reg, name, val);
}
